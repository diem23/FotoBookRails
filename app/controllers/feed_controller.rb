class FeedController < ApplicationController
    ...
    def photo
      if user_signed_in?
        ...
        @pagy,@list_photos =  pagy(@list_following.joins(:photos).select('users.*,photos.*').where(photos: {isPrivate: false}).order('photos.created_at DESC'))    
      end
    end
    ...
    def album
      if user_signed_in?
        ...
        @pagy,@list_albums = pagy(Album.where(user_id: @list_following.pluck(:id)).where(isPrivate: false).includes(:user).includes(:photos).order('created_at DESC'))
      end
    end
  end
